...
*/
function getLosses(){
  var lost = film.boxoffice - film.budget;
  return lost;   // Your answer here:
}
...
function interleave(){
    let vals = [];
    for(var i = 0; i < numberVals.length, i < letterVals.length; i++){
    vals.push(letterVals[i] + numberVals[i]);
      // console.log(letterVals[i] + numberVals[i]);
    }
